...
function buildIllustrationUrl(imageDescription, pageNum, { width = 800, height = 800 } = {}) {
  // imageDescription은 50단어 이내로 Gemini가 생성 → URL 길이 문제 없음
  const desc = imageDescription || `children storybook illustration scene ${pageNum}`
  return (
    `https://image.pollinations.ai/prompt/${encodeURIComponent(desc)}` +
    `?width=${width}&height=${height}&seed=${pageNum}`
  )
}
...
function StoryImage({ src, alt, className, fallbackIndex = 0 }) {
    ...
    <div className={`relative overflow-hidden bg-amber-50 ${className}`}>
      ...
      {status === 'loading' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-rose-50 to-amber-50">
          ...
          <p className="text-xs text-gray-300 mt-1">최대 40초 소요될 수 있어요</p>
        </div>
      )}
            ...
            {pages.map((p, i) => (
              ...
              >
                <div className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors
                  ...
                  <ThumbImage
                    src={buildIllustrationUrl(p.imageDescription, p.pageNumber, { width: 128, height: 128 })}
